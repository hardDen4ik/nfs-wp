...
class THWCFE_Public_Checkout extends THWCFE_Public {
	...
	private function enqueue_scripts($suffix, $jquery_version, $in_footer) {
		...
		$required_validation = apply_filters('thwcfe_required_validation_text', __('This field is mandatory'));
		$email_validation = apply_filters('thwcfe_email_validation_text', __('Invalid email format'));
		$phone_validation = apply_filters('thwcfe_phone_validation_text', __('Please enter the input in digits'));
		$url_validation = apply_filters('thwcfe_url_validation_text', __('Invalid url format'));
		$minlength_validation = apply_filters('thwcfe_minlength_validation_text', __('Text entered is less than the minimum length'));
		$number_validation = apply_filters('thwcfe_number_validation_text', __('Input value is not a number'));
		$wcfe_var = array(
			'lang' => array(
						'am' => THWCFE_i18n::t('am'),
						'pm' => THWCFE_i18n::t('pm'),
